...
::
:: This script automatically runs and tests the backend system
...
for %%t in (delete withdraw) do (

	:: Create the current categories file structure.
	mkdir "%out_dir%/%%t" >NUL
	mkdir "%out_dir%/%%t/newmaster" >NUL
  mkdir "%out_dir%/%%t/logs" >NUL
  mkdir "%out_dir%/%%t/output" >NUL
	mkdir "%out_dir%/%%t/accounts" >NUL

	:: Loop through the tests in the current category.
	for %%f in (.\tests\%%t\summary\*) do (
	
		:: Run the tests and compare the files.
		echo Running: %%~nf
		python ./scripts/main.py .\tests\%%t\oldmaster\oldmaster_%%~nf.txt .\tests\%%t\summary\%%~nf.txt %out_dir%/%%t/newmaster/newmaster_%%~nf.log %out_dir%/%%t/accounts/accounts_%%~nf.log < %%f >> %out_dir%/%%t/output/output_%%~nf.log
		python ./backendfileCompare.py %%~nf %out_dir%
